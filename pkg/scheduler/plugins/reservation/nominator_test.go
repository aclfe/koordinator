...
func TestNominateReservation(t *testing.T) {
	...
	// reservation2C4GPreferred is feasible and carries an explicit reservation order, which the normal path
	// prefers but first-fit ignores.
	...
	tests := []struct {
		...
		{
			// First-fit exhausts all matched reservations and nominates none when every one is infeasible.
		...
		{
			// First-fit returns the first matched feasible reservation even when a later one declares a more
			// preferred reservation order, which the best-fit path would otherwise select.
